...
public final class ArrayQueue<T> implements QueueInterface<T> {
	...
	/*public ArrayQueue(ArrayQueue<T> otherOther) {
		checkIntegrity();
		ensureCapacity();
		int otherLength = otherOther.queue.length;
		T[] newQueue = (T[]) new Object[otherLength];
		int otherFrontIndex = otherOther.frontIndex;
		int otherBackIndex = otherOther.backIndex;

		while (this.frontIndex <= otherFrontIndex) {
			this.queue[frontIndex+1] =
	...
	public void splice(ArrayQueue<T> secondQueue) {
		for (int i = secondQueue.frontIndex; i != (secondQueue.backIndex+1) % secondQueue.queue.length;
			 i = (i+1) % secondQueue.queue.length) {
