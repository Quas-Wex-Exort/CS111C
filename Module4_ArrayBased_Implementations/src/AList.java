...
*/
    ...
    public void add(int givenPosition, T newEntry)
    {
        ...
        if ((givenPosition >= 1) && (givenPosition <= numberOfEntries + 1))
        {
            ...
            throw new IndexOutOfBoundsException("Given position of add's new entry is out of " +
                    "bounds.");
    } // end add
    ...
    /*public int removeEvery(T element) {
        ...
        if (!this.isEmpty() && this.contains(element)) {
            while (this.getPosition(element) > 0) {
                remove(this.getPosition(element));
                timesRemoved++;
            }
            return timesRemoved;
        }
    ...
    /*
    calling getPosition inside a loop that calls getPosition is inefficient- store that value
    in a local variable instead; or, even better, directly access the array for a more
    efficient solution
    */
    ...
    /*
    the best way to use generics in casting is AList<?> otherList = (AList<?>) other;
     */

    /*
    @Override
    public int compareTo(AList<T> otherList) {
        int lengthCompare = this.getLength().compareTo(otherList.getLength());
        if ( lengthCompare == 0) {
            for (int i = 1; i <= this.getLength(); i++) {
                if (!this.getEntry(i).equals(otherList.getEntry(i))) {
                    return Integer.compare((int) this.getEntry(i), (int) otherList.getEntry(i));
                }
            }
        } else {
