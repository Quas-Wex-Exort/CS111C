...
public class EfficiencyExamples {
	...
	// The execution time is unaffected by the size of the arraylist, no matter how big it gets.
	public static int getMiddleValue(ArrayList<Integer> numberList) {
		...
		return numberList.get(middleIndex); // get method is constant time- per the ArrayList
		// API description
	...
	public static int calculateSum(ArrayList<Integer> numberList) {
		...
		for(int i=0; i<numberList.size(); i++) { // loop is dependent on the size of the list
			// dependent on a linear way, direct proportionality!
			sum += numberList.get(i); // get method is constant O(1)
			// get() method is not dependent on the size of the list!
		}
		return sum;
	}
	...
	// for example, if we double the size of our data (data x2), the running time might increase by
	// a factor of 4 (time increase by 2^ or 2x2 = 4)
	// note: this method can be made more efficient from a real-world perspective
	// (although it will still be O(n^2))
