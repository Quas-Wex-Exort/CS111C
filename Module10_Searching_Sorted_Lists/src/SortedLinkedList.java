...
public class SortedLinkedList<T extends Comparable<? super T>> implements
	...
	public int countEntries(T entry) {
		...
		// 		break;
		// 	} else if (currentNode.data.equals(entry)) {
		// 		count++;
		// 	}
		// 	currentNode = currentNode.next;
		// }
		// return count;
		return countEntriesHelper(firstNode, entry);
	}

	private int countEntriesHelper(Node currentNode, T entry) {
		if (currentNode == null || currentNode.data.compareTo(entry) > 0) {
			return 0;
		} else {
			boolean check = currentNode.data.equals(entry);
			return check ? 1 + countEntriesHelper(currentNode.next, entry) : countEntriesHelper(currentNode.next, entry);
		}
	}
