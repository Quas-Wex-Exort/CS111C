...
public class AList<T extends Comparable<? super T>> implements ListInterface<T>, Comparable<AList <T>> {
    ...
    /**
     * @param anObject
     * @return The first integer position if anObject is found, otherwise,
     * returns -1.
     */
    public int getPosition(T anObject) {
        ...
        for (int i = 1; i <= numberOfEntries; i++) {
            if (this.list != null && anObject.equals(this.list[i])) {
                return i; // stops loop if anObject is found
            }
        }
    ...
    public int compareTo(AList<T> otherList){
        ...
        if (firstArray.length != otherArray.length) {
            return (firstArray.length > otherArray.length) ? 1 : -1;
        } else {
            int firstArrayCount = 0, otherArrayCount = 0;
            for (int i = 0; i < firstArray.length; i++){
                if (firstArray[i].compareTo(otherArray[i]) > 0){
                    firstArrayCount++;
                } else if (firstArray[i].compareTo(otherArray[i]) < 0){
                    otherArrayCount++;
                }
            }
