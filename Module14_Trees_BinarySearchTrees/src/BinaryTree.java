...
public class BinaryTree<T> implements BinaryTreeInterface<T> {
	...
	public int count(T anEntry) {
		int count = 0;
		Iterator <T> iterator = this.getLevelOrderIterator();
	...
	*/
	...
	public int count(T anEntry) {
		return countHelperMethod(root, anEntry);
	}

	private int countHelperMethod(BinaryNode<T> currentNode, T anEntry) {
		int count = 0;
		if (currentNode != null) {
			...
			return count + countHelperMethod(currentNode.getLeftChild(), anEntry) + countHelperMethod(currentNode.getRightChild(), anEntry);
	...
	private int countHelperMethod(BinaryNode<T> node, T anEntry) {
		if (node != null) {
			int count = 0;
			...
			return count + countHelperMethod(leftChild, anEntry) + countHelperMethod(rightChild,
					anEntry);
		} else {
			return 0;
		}
